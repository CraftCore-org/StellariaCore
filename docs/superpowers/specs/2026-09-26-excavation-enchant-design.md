...
2. 殴った面を求める。`player.rayTraceBlocks(<ブロック到達距離>)` で視線の先を判定し、ヒットしたブロックが壊したブロックと一致すればその面を使う。一致しない、または判定できない場合は範囲破壊をせず中心だけにする。
3. `ExcavationRules.offsets(face)` で、面に垂直な平面上の周囲 8 マスを求める。
   - `UP`/`DOWN` を殴った場合は X-Z 平面
   - `NORTH`/`SOUTH` を殴った場合は X-Y 平面
   - `EAST`/`WEST` を殴った場合は Z-Y 平面
4. 周囲の各ブロックについて、4.3 の条件を満たすものだけを `player.breakBlock(block)` で壊す。
5. 1 ブロック壊すごとに道具の残り耐久値を確認し、`min-durability`（既定 10）を下回ったらそこで打ち切る。打ち切ったとき、または開始時点ですでに下回っていたときは、アクションバーに警告を出す（4.5）。
...
- 空気・液体ではない
- 手に持っている道具の適正ブロックである（`block.isPreferredTool(tool)`。ツルハシなら石系、シャベルなら土系）
- 硬さ（`getType().getHardness()`）が 0 以上で、中心のブロックの硬さ以下である。石を掘ったときに黒曜石まで壊れることを防ぐ。岩盤などの壊せないブロック（硬さが負）は対象外になる
- 中身を持つブロック（`BlockState` が `Container`）ではない。チェストやかまどの中身がばらまかれる事故を防ぐ
