...
- 統計ランキングの一覧には、値が 1 以上のプレイヤーだけを載せる（`pvpkills` などで 0 の人が並ばないようにするため）。
- 同じ値のプレイヤーは同じ順位にし、次の順位は人数ぶん飛ばす（1, 2, 2, 4）。ページをまたいで同じ値が続く場合も同じ順位になる。`money` と `playtime` も同じ規則で表示する。
